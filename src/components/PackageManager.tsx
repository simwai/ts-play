  ...
  return (
    <div className="border-t border-surface0 bg-mantle shrink-0">
      <PanelHeader
        ...
        isOpen={isOpen}
        onToggle={onToggle}
        left={packages.length > 0 ? <Badge label={String(packages.length)} /> : undefined}
      />
          ...
          {packages.length > 0 ? (
                ...
                >
                  <div className="flex-1 min-w-0 flex flex-col gap-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-text text-sm font-semibold font-mono">{pkg.name}</span>
            ...
            <div className="p-6 text-center text-overlay0 text-sm font-mono">
              ...
              <span className="opacity-70 text-xs">
                Type <code className="text-mauve">import React from 'react'</code> to see it appear
                here automatically.
              </span>
            </div>
          )}
