...
export const Console = React.memo(function Console({
  ...
  const converter = useMemo(() => new Ansi({ newline: true, escapeXML: true }), [])
  ...
  const filteredMessages = useMemo(() => {
    ...
    return messages.filter((m) => {
      ...
      if (filter !== 'all') {
        ...
        if (filter === 'info' && !['info', 'debug', 'dir'].includes(m.type)) return false
        if (filter === 'warn' && !['warn', 'trace'].includes(m.type)) return false
        if (filter === 'error' && m.type !== 'error') return false
      }
      if (searchLower && !m.args.some(a => String(a).toLowerCase().includes(searchLower))) return false
      return true
    })
  ...
  const FilterButton = ({ type, label }: { type: FilterType, label: string }) => (
    <button
      onClick={(e) => { e.stopPropagation(); setFilter(type); }}
      className={cn(
        "px-2 py-0.5 rounded text-[10px] font-bold uppercase transition-colors",
        filter === type ? "bg-lavender/20 text-lavender" : "text-subtext1 hover:text-text"
      )}
    >
      {label}
    </button>
  ...
  const TabButton = ({ id, label, count, variant }: { id: any, label: string, count?: number, variant?: BadgeVariant }) => (
    <button
      onClick={(e) => { e.stopPropagation(); onTabChange(id); if (!isOpen) onToggle(); }}
      className={cn(
        "flex items-center gap-1.5 px-3 py-1.5 rounded-md transition-all",
        activeTab === id ? "bg-surface0 text-lavender shadow-sm" : "text-subtext1 hover:text-text"
      )}
    >
      <span className="text-[10px] font-bold uppercase tracking-wider">{label}</span>
      {count !== undefined && count > 0 && (
        <Badge label={String(count)} variant={variant} className="scale-90" />
      )}
    </button>
  )
  ...
  return (
    <div className="flex flex-col border-t border-surface0 bg-mantle shrink-0 overflow-hidden h-full" data-testid="console-container">
      <div
        className="flex items-center justify-between px-2 h-10 transition-colors bg-mantle/50 border-b border-surface0/30 cursor-pointer hover:bg-surface0/20"
        onClick={onToggle}
      >
        <div className="flex items-center gap-1">
          <TabButton id="console" label="Console" count={messages.length} />
          <TabButton id="problems" label="Problems" count={problemCount} variant={problemCount > 0 ? 'error' : 'default'} />
        ...
        <div className="flex items-center gap-3 pr-2">
          ...
          <ChevronDown className={cn("w-4 h-4 text-overlay0 transition-transform duration-300", isOpen && "rotate-180")} />
        </div>
      </div>
          ...
          {activeTab === 'console' && (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-base/30 border-b border-surface0/20">
              ...
              <div className="relative flex-1 max-w-xs ml-2">
                <Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-overlay0" />
                <input
                  value={search}
                ...
                />
                {search && <X size={10} className="absolute right-2 top-1/2 -translate-y-1/2 text-overlay0 cursor-pointer hover:text-text" onClick={() => setSearch('')} />}
              </div>

              <div className="flex items-center gap-2 ml-auto">
                 {errors > 0 && <Badge label={`${errors} ERR`} variant="error" />}
                 {warns > 0 && <Badge label={`${warns} WRN`} variant="warn" />}
              </div>
            </div>
          )}
            ...
            {activeTab === 'console' ? (
              filteredMessages.length === 0 ? (
                ...
                filteredMessages.map((msg, i) => {
                  ...
                  return (
                    <div key={`${msg.ts}-${i}`} className={cn(
                      "flex items-start gap-2.5 py-1.5 px-2 border-b border-surface0/30 last:border-0 hover:bg-surface0/5 transition-colors",
                      msg.type === 'error' ? 'bg-red/5' : msg.type === 'warn' ? 'bg-yellow/5' : ''
                    )}>
                      <Badge label={typeLabel(msg.type)} variant={typeVariant(msg.type)} className="h-4 px-1 py-0 mt-0.5 min-w-[32px] justify-center" />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="whitespace-pre-wrap break-all text-subtext1 leading-relaxed pl-1">
                          {hasAnsi ? (
                            <span dangerouslySetInnerHTML={{ __html: converter.toHtml(fullText) }} />
                          ) : (
                            fullText
                          )}
                        </div>
