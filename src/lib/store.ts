...
export const preferredDarkThemeAtom = atomWithStorage<ThemeMode>('tsplay_dark_theme', 'mocha' as any)
export const preferredLightThemeAtom = atomWithStorage<ThemeMode>('tsplay_light_theme', 'latte' as any)
...
export const enqueueTaskAtom = atom(
  null,
  async (_get, set, { name, task }: { name: string, task: () => Promise<any> }) => {
    const id = Math.random().toString(36).slice(2, 11)
    const toastId = id
    set(toastsAtom, (prev) => [...prev, { id, type: 'info' as ToastType, message: `Action queued: ${name}` }])
    ...
    const newPromise = currentQueue.then(async () => {
      ...
      set(toastsAtom, (prev) => [...prev, { id: failId, type: 'error' as ToastType, message: `Task failed: ${name}` }])
    })
