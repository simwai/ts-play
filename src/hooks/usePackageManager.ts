...
export function usePackageManager(
  ...
  const checkImports = useCallback(() => {
    ...
    checkImportsTimeout.current = setTimeout(async () => {
      ...
      try {
        ...
        const filtered = [...detected].filter(pkg => !pkg.startsWith('node:') && !BUILTIN_MODULES.has(pkg))
  ...
  useEffect(() => { checkImports() }, [tsCode, checkImports])
    ...
    const performChanges = async () => {
       ...
       try {
          ...
          if (added.length > 0) {
            ...
            await runCommand('npm', ['install', '--no-progress', ...added], (out) => {
                const clean = out.replaceAll(/\u001B\[[\d;]*[a-zA-Z]/g, '').trim()
                if (clean && !/^[/\\|\-]$/.test(clean)) addMessage('info', [clean])
            })
          }
  ...
  return { installedPackages, packageTypings, tsCursorPos, checkImports, installQueue, status }
}
