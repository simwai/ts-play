...
// `lsVersion` tracks the TS compiler version we built `ls` against;
// if the compiler reloads (e.g. hot-reload) we rebuild the service.
let ls: any = null;
let lsVersion = '';
let libsSignature = '';
...
export function useTSDiagnostics(
  ...
  useEffect(() => {
    ...
    timerRef.current = window.setTimeout(() => {
      ...
      // If code is huge, skip diagnostics to keep typing smooth.
      ...
      schedule(async () => {
        try {
          // Rebuild the language service if the compiler was hot-reloaded
          ...
          if (!ls) {
            // Verify all required TS APIs exist
          ...
          try {
            ...
            // Suppress errors that only appear because we don't ship lib.d.ts.
            const ignoredCodes = new Set([
              1128, // Declaration or statement expected (spurious in snippet ctx)
              2308, // Module X has already exported a member named Y
            ]);
            ...
            const formatted: TSDiagnostic[] = filtered.map((d: any) => {
              ...
              // Compute line/character from char offset
